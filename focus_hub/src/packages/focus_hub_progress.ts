...
import { KIND_LABEL, readRecentProgress, recordProgress } from "../shared/progress.js";
...
export async function report_progress(params: {
  ...
  user_quote?: string;
  note?: string;
}): Promise<{ success: boolean; status: string; message: string; id?: string }> {
  try {
    ...
    const label = KIND_LABEL[result.record.kind];
...
export async function get_recent_progress(params: { limit?: number | string }): Promise<{ success: boolean; records: string }> {
  const limit = Math.min(Math.max(Number(params?.limit) || 10, 1), 50);
  try {
    const records = await readRecentProgress(limit);
    if (records.length === 0) return { success: true, records: "今天和昨天没有进展记录" };
    const lines = records.map(
      (r) => `${r.iso}｜${KIND_LABEL[r.kind] ?? r.kind}｜「${r.user_quote}」${r.note ? `｜${r.note}` : ""}｜任务 ${r.task || "未知"}｜来自 ${r.via === "dashboard" ? "主控台" : "对话"}`
    );
