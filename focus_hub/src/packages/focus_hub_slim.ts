...
import { collectSlimReport, scanRedundancy, slimReportToText } from "../shared/slim.js";

export async function get_slim_report(params: { scan?: boolean | string } = {}): Promise<{ success: boolean; report: string }> {
  const report = await collectSlimReport();
  let text = slimReportToText(report);
  if (params.scan === true || params.scan === "true") {
    const scan = await scanRedundancy();
    text += `\n冗余候选 ${scan.candidates.length} 个（清单写在 ${scan.reportPath}）：${scan.candidates.slice(0, 12).map((c) => c.name).join("、") || "无"}`;
    if (scan.workflows.length) text += `\n可以考虑停用的工作流：${scan.workflows.map((w) => `${w.name}（${w.why}）`).join("；")}`;
  }
