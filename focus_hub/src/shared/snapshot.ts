export const FIXED_CHAT_TITLE = "主控台";
...
async function loadScheduleIntervalMs(workflowId: string): Promise<number | null> {
  const detail = await Tools.Workflow.get(workflowId);
  for (const node of detail.nodes ?? []) {
    const trigger = node as { type?: string; triggerType?: string; triggerConfig?: Record<string, string> };
    if (trigger.type !== "trigger" || trigger.triggerType !== "schedule") continue;
    const config = trigger.triggerConfig ?? {};
    if (config.enabled === "false") continue;
    if (config.schedule_type !== "interval") return null;
