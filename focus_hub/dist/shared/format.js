...
exports.SOURCE_LABEL = exports.HEALTH_LABEL = void 0;
...
const mood_js_1 = require("./mood.js");
function moodOf(snap) {
    return (0, mood_js_1.computeMood)({
...
function workflowLine(row, now) {
    const parts = [exports.HEALTH_LABEL[row.health]];
...
function snapshotToText(snap) {
    ...
    const out = [];
    out.push(`【主控台快照】生成于 ${(0, snapshot_js_1.formatDateTime)(now)}`);
    out.push("", "■ 当前任务（task_state.txt）");
    if (snap.task) {
        out.push(`${snap.task.task || "未记录"}｜状态 ${snap.task.state || "未记录"}｜模式 ${snap.task.mode || "未记录"}｜同步于 ${snap.task.syncedAt || "未记录"}`);
    }
    else {
        out.push("未读取到");
    }
    ...
    if (mood.pendingCheckin)
        out.push(`${mood.pendingCheckin.iso.slice(11, 16)} 打卡过一次，用户还没回应：「${mood.pendingCheckin.line}」`);
    const focus = snap.events?.focus;
    if (focus) {
        const known = focus.onTask + focus.offTask;
        out.push("", `■ 今日专注（前台采样口径，不是时长）：在任务上 ${focus.onTask}/${known}${known > 0 ? `（${Math.round((focus.onTask / known) * 100)}%）` : ""}，未知 ${focus.unknown}`);
        if (focus.offApps.length > 0)
            out.push(`偏离时最常开：${focus.offApps.map((a) => `${a.name} ${a.count} 次`).join("、")}`);
    }
    ...
    if (snap.usage && snap.usage.rows.length > 0) {
        for (const row of snap.usage.rows) {
            out.push(`- ${row.appName}：${row.foregroundMinutes} 分钟，最近系统记录 ${(0, snapshot_js_1.formatClock)(row.lastTimeUsed)}`);
        }
    }
    else {
    ...
    if (snap.events) {
        ...
        if (snap.events.unparsable > 0)
            out.push(`（另有 ${snap.events.unparsable} 行无法解析）`);
    }
    else {
        out.push("未读取到");
    }
    ...
    if (snap.actions && snap.actions.rows.length > 0) {
        for (const cells of snap.actions.rows.slice(0, 8))
            out.push(`- ${cells.join(" · ")}`);
    }
    else {
    ...
    for (const h of snap.health) {
        const age = h.modifiedAt != null ? (0, snapshot_js_1.formatAgo)(h.modifiedAt, now) : "—";
    ...
    for (const source of snap.sources) {
        out.push(`- ${source.label}：${exports.SOURCE_LABEL[source.status]}${source.status === "OK" ? "" : `（${source.detail}）`}`);
    }
    return out.join("\n");
}
